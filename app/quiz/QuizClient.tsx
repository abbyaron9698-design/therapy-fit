...
const RESULTS_LS_KEY = "tf_results_r";

function selectedMulti(answers: Answers, qid: keyof Answers): string[] {
  const v = answers[qid];
  return Array.isArray(v) ? v : [];
}

function computeTier1Gates(answers: Answers): ResultsPayloadV1["gates"] {
  const symptoms = selectedMulti(answers, "symptoms");
  const distressFirst = selectedMulti(answers, "distressFirst");

  const hasDissociationSignal = distressFirst.includes("dissociate");
  const hasShutdownSignal = distressFirst.includes("shutdown");
  const hasNumbSymptom = symptoms.includes("numb");
  const hasOverwhelm =
    symptoms.includes("overwhelm") || distressFirst.includes("flooded");
  const hasPanic = symptoms.includes("panic");

  const stabilizationFirst =
    hasDissociationSignal ||
    hasShutdownSignal ||
    hasNumbSymptom ||
    (hasOverwhelm && hasPanic);

  const hasTrauma = symptoms.includes("trauma_reminders");
  const traumaProcessingReady = hasTrauma && !stabilizationFirst;

  const gates: NonNullable<ResultsPayloadV1["gates"]> = {};
  if (stabilizationFirst) gates.stabilizationFirst = true;
  if (traumaProcessingReady) gates.traumaProcessingReady = true;
...
export default function QuizClient() {
  ...
  function submit() {
    ...
    const gates = computeTier1Gates(answers);
    ...
    // ✅ Save results so the /results tab works even without URL params
    try {
      localStorage.setItem(RESULTS_LS_KEY, encoded);
    } catch {
      // ignore
    }
        ...
        <div className="rounded-3xl border bg-white p-8 shadow-sm">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-slate-600">
                Answer a few questions about what you’re experiencing and how you
                learn. You can select multiple options—there’s no limit.
              </p>
            </div>
          ...
          <details className="mt-6 rounded-2xl border bg-slate-50 p-4">
            ...
            <div className="mt-3 text-sm text-slate-700">
              We look for patterns across what you’re experiencing (e.g.,
              avoidance, emotional overwhelm, trauma reminders) and how you
              prefer to learn (tools, exploration, creative expression). Then we
              generate a short list of therapy approaches and explain why they
              may fit.
            </div>
          </details>
        </div>
