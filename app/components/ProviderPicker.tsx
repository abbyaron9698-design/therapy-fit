// app/components/ProviderPicker.tsx
"use client";
...
export default function ProviderPicker({ modalityId, title }: Props) {
  ...
  const languageOptions = useMemo(() => {
    ...
    // LanguageOption already supports "Any"; extras are valid strings too for UI purposes
  ...
  const filtered = useMemo(() => {
    ...
    return PROVIDERS.filter((p: Provider) => {
      if (!p.modalities?.includes(modalityId)) return false;
      ...
      // NOTE: here ZIP is used as an actual filter (unlike ProviderDirectory).
      if (z.length >= 3) {
  ...
  }, [zip, insurance, language, telehealthOnly, acceptingOnly, modalityId]);
    ...
    <div className="mt-6 rounded-2xl border bg-white p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="text-sm font-medium text-slate-900">
            {title ?? "Providers (starter Chicagoland list)"}
          </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="grid gap-1">
          ...
          <span className="text-[11px] text-slate-500">More payers coming</span>
        </label>
        ...
        <label className="grid gap-1">
          ...
          <span className="text-[11px] text-slate-500">We’ll keep expanding.</span>
        </label>
